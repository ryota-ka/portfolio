...
class Timestamp extends React.Component<Pick<Track, 'timestamp'>> {
    render() {
        ...
        if (timestamp === null) {
            return <span className="label green"><i className="fa fa-headphones"></i> now playing</span>;
        } else {
            return <span className="label gray"><i className="fa fa-clock-o"></i> {format(timestamp * 1000, 'YYYY-MM-DD HH:mm:ss')}</span>;
        }
    }
}
...
class Loved extends React.Component<{ loved: Track['loved'] }> {
    render() {
        if (this.props.loved) {
            return <span className="label red"><i className="fa fa-heart"></i> loved</span>
        } else {
            return null;
        }
    }
